...
-- Run in Supabase SQL editor after clearing old schema.

-- Extensions
create extension if not exists pgcrypto;
create extension if not exists postgis;

-- Profiles: ZIP discovery
...
-- Track claim changes for yearly quota
...
-- Posts: explicit expires_at (no generated column)
...
-- Enforce max 3 active claims + 3 changes per year
...
using (
  ...
  and hidden = false
  and (
    public.within_claim_radius(geometry)
    or public.within_presence_radius(geometry)
  )
);
...
with check (
  ...
  and expires_at > now()
  and (
    -- If user has a claim, post must be within claim radius
    (public.has_active_block_claim() and public.within_claim_radius(geometry))
    or
    -- If no claim, allow only status/ask within 1 mile
  ...
  and (
    -- Status: 24h if claimed, 1h if unclaimed (handled above)
    (post_type = 'status' and duration_minutes in (60, 1440))
    or
    -- Ask: 1h to 7d
